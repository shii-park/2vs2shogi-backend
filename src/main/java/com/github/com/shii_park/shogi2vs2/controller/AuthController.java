...
public class AuthController {
    ...
    public LoginResponse register(@RequestBody @Valid LoginRequest request) {
        ...
        String sessionId = UUID.randomUUID().toString(); // ランダムなユーザーID生成
        System.out.println("[AuthController] ユーザーIDを生成しました。ユーザーID=" + sessionId);

        redisTemplate.opsForValue().set(
                "user:" + sessionId,
                request.getUsername(),
                Duration.ofHours(2));
        System.out.println("[AuthController] ユーザー情報を保存しました。キー=user:" + sessionId + ", TTL=2時間");

        return new LoginResponse(sessionId, request.getUsername());
    }
}
